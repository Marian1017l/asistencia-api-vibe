...
const registrarAsistencia = (dto) => {
  ...
  if (!estudianteExiste) return { error: `Estudiante con ID ${dto.estudianteId} no encontrado` };

  const duplicado = asistencias.find(
    (a) => a.estudianteId === dto.estudianteId && a.fecha === dto.fecha
  );
  if (duplicado) return { error: `Ya existe un registro de asistencia para ${dto.estudianteId} en la fecha ${dto.fecha}` };

  const nueva = { id: uuidv4(), ...dto, creadoEn: new Date().toISOString() };
